...
describe("cross", () => {
    ...
    it("Cross install", async () => {
        const spy = vitest.spyOn(io, "which").mockResolvedValueOnce("/home/user/.cross/bin/cross");
        const spy2 = vitest.spyOn(process, "cwd").mockReturnValueOnce("/somewhere/on/the/machine");
        const spy3 = vitest.spyOn(process, "chdir").mockReturnValue(undefined);
    ...
    it("Cross getOrInstall fail", async () => {
        const spy = vitest.spyOn(io, "which").mockResolvedValueOnce("/home/user/.cross/bin/cross");
        const spy2 = vitest.spyOn(Cross, "get").mockRejectedValue(new Error("Not found"));
        const spy3 = vitest.spyOn(core, "debug");
